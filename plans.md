...
- [x] マイルストーン 2: AST + パーサ
- [x] マイルストーン 3: 評価器
- [x] マイルストーン 4: REPL + ランナー
- [x] マイルストーン 5: 受け入れ

## 方針転換（CPython互換優先）
- [x] 開発方針を「MVPスコープ維持」から「可能な限りCPython互換を目指す」へ切替
- [x] 優先対象を言語セマンティクス互換（名前解決/属性解決/実行順）に設定
- [x] C1: 名前解決セマンティクス互換（LEGB/NameError境界/既存回帰増強）
- [x] C2: 属性解決・呼び出しセマンティクス互換（attribute lookup/bound method/MRO）
- [x] C3: 実行順・評価順互換（引数評価順/短絡/副作用/例外優先順位）
- [x] C4: import/runtime セマンティクス補強（module初期化可視性/sys.modules 相当）
- [x] C5: ドキュメント非MVP化（README/plans を互換ギャップ駆動へ更新）
...
- [x] テスト可読性改善: `RuntimeErrorSpec` の関数呼び出し引数エラー巨大ケースを仕様単位へ分割
- [x] `float` 最小導入（literal: `1.23` / `1.` / `.5` / 指数表記、演算: `/` 実数除算・`//` 床除算、混在比較の暗黙昇格）
- [x] `float` 導入に伴う既存仕様差分の明文化（`/` 系期待値更新、表示規約、ゼロ除算エラー整合）

## 現在のスコープ（P20: 高優先機能バックログ整理）
- [x] P20 開始: 未対応機能を「実装優先度」で再整理するスコープを開始
- [x] 優先度方針を固定（ユーザー価値 / 実装リスク / 既存VM基盤との接続容易性）
- [x] P0-1 を `汎用 import 拡張` として確定（`math`/ローカル `.py` 以外への導線拡張）
- [x] P0-1 実装: 失敗テスト先行で import 解決対象・探索順・エラー規約を固定（`pkg/__init__.py` 解決を追加）
- [x] P0-2 を `高度な例外処理` として確定（型指定 `except` など）
- [x] P0-2 実装: 失敗テスト先行で `except <Type> as <name>` と複数節の選択規則を導入（`RuntimeError/TypeError/...` を型名にマップ）
- [x] P1-1 を `match/case 互換拡張` として確定（既存MVP差分の穴埋め）
- [x] P1-1 実装: `Pattern` / VM 実行の不足ケースを parity テストで固定（`as-pattern` を parser/VM 実装へ追加）
- [x] P1-2 を `一般メソッド解決拡張` として確定（組み込み糖衣以外）
- [x] P1-2 実装: インスタンス属性/メソッド探索規則を最小導入（`x.method` の bound method 参照と呼び出しを追加）
- [x] P2 を `RuntimeErrorSpec 分割` として確定（保守性改善）
- [x] P2 実装: 巨大ケースを仕様単位へ分割し、テスト名を責務ごとに再編

## 現在のスコープ（P21: match/case 互換追加）
- [x] P21 開始: `match/case` の mapping pattern 互換を段階拡張
...
5. [x] 実装計画: `with` 文（Context Manager）の最小実装（lex/parse/VM）
