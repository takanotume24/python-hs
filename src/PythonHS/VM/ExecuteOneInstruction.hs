  ...
  case instruction of
    ...
    StoreName name ->
      case vmStack state of
        value : rest ->
          case storeNameWithAttr (vmIsTopLevel state) (envGlobalDecls (vmEnv state)) name value (envGlobals (vmEnv state)) (envLocals (vmEnv state)) of
            Left err -> Left err
    ...
    MatchPattern pattern _ -> do
      (newStack, newGlobals, newLocals) <-
        executeMatchPattern (vmIsTopLevel state) (envGlobalDecls (vmEnv state)) pattern (vmStack state) (envGlobals (vmEnv state)) (envLocals (vmEnv state))
      let newEnv = (vmEnv state) {envGlobals = newGlobals, envLocals = newLocals}
    ...
    ForNext name loopEndIndex _ ->
      case executeForNext (vmIsTopLevel state) (envGlobalDecls (vmEnv state)) (vmIp state) name loopEndIndex (loopForStates (vmLoop state)) (envGlobals (vmEnv state)) (envLocals (vmEnv state)) of
        Left err -> Left err
    ...
    UnpackToNames names pos ->
      case vmStack state of
        value : rest ->
          case executeUnpackToNames (vmIsTopLevel state) (envGlobalDecls (vmEnv state)) pos names value (envGlobals (vmEnv state)) (envLocals (vmEnv state)) of
            Left err -> Left err
            Right (newGlobals, newLocals) ->
      ...
      case vmStack state of
        right : left : rest ->
          case evalBinaryOp op left right pos of
            Left err -> Left err
            Right value -> execute state {vmIp = vmIp state + 1, vmStack = value : rest}
        _ -> Left "VM runtime error: add requires two values on stack"
    ApplyUnaryMinus pos ->
      case vmStack state of
        value : rest ->
          case value of
            IntValue n -> execute state {vmIp = vmIp state + 1, vmStack = IntValue (negate n) : rest}
            FloatValue n -> execute state {vmIp = vmIp state + 1, vmStack = FloatValue (negate n) : rest}
            _ -> Left ("Type error: unary - expects int at " ++ showPos pos)
        _ -> Left "VM runtime error: unary - requires one value on stack"
    ApplyNot _ ->
      case vmStack state of
        value : rest -> execute state {vmIp = vmIp state + 1, vmStack = IntValue (if isTruthy value then 0 else 1) : rest}
