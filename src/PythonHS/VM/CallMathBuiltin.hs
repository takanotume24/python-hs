...
callMathBuiltin :: CallMathBuiltinConfig -> Maybe (Either String Value)
callMathBuiltin config = case name of
  ...
  _ -> Nothing
  where
    name = callMathBuiltinName config
    args = callMathBuiltinArgs config
    pos = callMathBuiltinPos config
